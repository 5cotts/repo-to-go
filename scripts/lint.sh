#!/usr/bin/env bash
# Derived from https://github.com/kensho-technologies/graphql-compiler/blob/main/scripts/lint.sh
...
diff_only=0
any_run_only_set=0
run_fast_linters=0  # isort, black, flake8, pydocstyle
run_pylint=0
run_mypy=0
for i in "$@"; do
    case $i in
...
lintable_locations="$(find . -name '*py' -print | grep -v alembic | grep -v migrations | grep -v tests)"
...
if [ "$diff_only" -eq 1 ] ; then
    ...
    lintable_locations="$(git diff --relative --name-only master... | grep -v alembic | grep -v migrations | grep ".*\.py$")"
...
if [ "$run_fast_linters" -eq 1 ]; then
    ...
    isort . --check-only --src . --settings-path=${ROOT_DIR}/ops/lint/lint_setup.cfg --diff
    isort_exit_code=$?
    echo -e "\n*** End of isort run; exit: $isort_exit_code ***\n"

    echo -e '*** Running black... ***\n'
    black --check --diff --skip-string-normalization --config ${ROOT_DIR}/ops/lint/black.toml .
    black_exit_code=$?
    echo -e "\n*** End of black run; exit: $black_exit_code ***\n"

    echo -e '*** Running flake8... ***\n'
    flake8 --config=${ROOT_DIR}/ops/lint/lint_setup.cfg $lintable_locations
    flake_exit_code=$?
    echo -e "\n*** End of flake8 run, exit: $flake_exit_code ***\n"

    #echo -e '\n*** Running pydocstyle... ***\n'
    #pydocstyle --config=${ROOT_DIR}/ops/lint/pydocstyle $lintable_locations
    #pydocstyle_exit_code=$?
    #echo -e "\n*** End of pydocstyle run, exit: $pydocstyle_exit_code ***\n"

    #echo -e '\n*** Running pydocstyle on tests... ***\n'
    #pydocstyle --config=${ROOT_DIR}/ops/lint/pydocstyle_test $lintable_locations
...
if [ "$run_mypy" -eq 1 ]; then
    echo -e '*** Running mypy... ***\n'
    mypy  --ignore-missing-imports --show-traceback $lintable_locations
    mypy_exit_code=$?
...
if [ "$run_pylint" -eq 1 ]; then
    ...
    pylint --jobs="$physical_core_count" --rcfile=${ROOT_DIR}/ops/lint/pylintrc $pylint_lintable_locations
    pylint_exit_code=$?
